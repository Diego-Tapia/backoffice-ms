...
export class ProcessMassiveDecreaseApplication implements IProcessMassiveDecreaseApplication {
  ...
  async execute(createMassiveDto: CreateMassiveDto, req: RequestModel) {
    ...
    if (!massiveId) throw new BadRequestException('massiveId is required');

    try {
      let massiveDecrease = await this.massiveDecreaseRepository.findById(massiveId);
      if (massiveDecrease.status !== EMassiveStatus.READY_PROCESS) throw new BadRequestException(`the status of the massive decrease should be ${ EMassiveStatus.READY_PROCESS }`);

      const token = await this.tokenRepository.findById(massiveDecrease.tokenId)
      ...
      massiveDecrease = await this.massiveDecreaseRepository.update({ _id: massiveId }, { status: EMassiveStatus.PROCESSING })
      ...
      if (massiveDecrease.recordLengthValidatedOk === 0) massiveDecrease.status = EMassiveStatus.INVALID
      return await this.massiveDecreaseRepository.update({ _id: massiveId }, massiveDecrease);  
